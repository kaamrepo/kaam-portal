...
const DropdownUser = () => {
  ...
  const getInitialProfile = (firstname: string, lastname: string) => {
  ...
  const initials = getInitialProfile(user.firstname || "", user.lastname || "");
  useEffect(() => {
    ...
    <div className="relative">
      ...
      >
        {user && user.profilepic ? (
          ...
          <div
            className={`h-12 w-12 text-lg font-bold rounded-full flex items-center justify-center text-white ${randomColorClass}`}
          >
            {initials}
          </div>
        )}
